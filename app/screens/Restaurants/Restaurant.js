...
export default class Restaurant extends Component {
  ...
  checkAddReviewUser = () => {
    ...
    const idEmergency = this.props.navigation.state.params.restaurant.item
      .restaurant.id;
  ...
  loadReviews = async () => {
    ...
    const reviews = db.collection("reviews").where("idEmergency", "==", id);
  ...
  render() {
    ...
    const listExtraInfoDateTime = [
      {
        text: `${new Date((createAt['seconds'])).toUTCString()}`,
        iconName: "date-range",
    ...
    return (
      <ScrollView style={styles.viewBody}>
        ...
        <View style={styles.viewRestaurantExtraInfo}>
          ...
          {listExtraInfoCoords.map((item, index) => (
            <ListItem
              key={index}
              title={item.text}
          ...
          {listExtraInfoDateTime.map((item, index) => (
            <ListItem
              key={index}
              title={item.text}
              leftIcon={<Icon name={item.iconName} type={item.iconType} />}
